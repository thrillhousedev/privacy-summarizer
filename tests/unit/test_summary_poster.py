...
class TestResendSummary:
    """Tests for resend_summary method."""

    def test_resends_existing_summary(self):
        """Resends a previously generated summary."""
        mock_cli = MagicMock(spec=SignalCLI)
        mock_repo = MagicMock(spec=DatabaseRepository)

        # Setup summary run mock
        mock_run = MagicMock()
        mock_run.id = 1
        mock_run.summary_text = "Previous summary text"
        mock_run.schedule = MagicMock()
        mock_run.schedule.name = "Test Schedule"
        mock_run.schedule.target_group = MagicMock()
        mock_run.schedule.target_group.name = "Target"
        mock_run.schedule.target_group.group_id = "target-id"

        mock_repo.get_recent_summary_runs.return_value = [mock_run]
        ...
        result = poster.resend_summary(run_id=1)

        assert result is True
        mock_cli.send_message.assert_called()

    def test_dry_run_does_not_resend(self):
        """Dry run prints but doesn't resend."""
        mock_cli = MagicMock(spec=SignalCLI)
        mock_repo = MagicMock(spec=DatabaseRepository)

        mock_run = MagicMock()
        mock_run.id = 1
        mock_run.summary_text = "Previous summary"
        mock_run.schedule = MagicMock()
        mock_run.schedule.target_group = MagicMock()
        mock_run.schedule.target_group.name = "Target"
        ...
        result = poster.resend_summary(run_id=1, dry_run=True)

        assert result is True
        mock_cli.send_message.assert_not_called()

    def test_run_not_found_fails(self):
        """Returns False when run not found."""
        mock_repo = MagicMock(spec=DatabaseRepository)
        mock_repo.get_recent_summary_runs.return_value = []
        ...
        result = poster.resend_summary(run_id=999)
